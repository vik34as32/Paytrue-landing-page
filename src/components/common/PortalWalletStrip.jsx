...
export default function PortalWalletStrip({
  ...
  const [commissionLoading, setCommissionLoading] = useState(Boolean(commissionHref));

  useEffect(() => {
    if (!commissionHref) return;
    let cancelled = false;
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) {
          setCommissionBalance(data.availableBalance ?? data.balance ?? 0);
        }
      } catch {
        if (!cancelled) setCommissionBalance(null);
      } finally {
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap items-end gap-6 text-right">
            ...
            {commissionHref ? (
              <Link
                href={commissionHref}
                className="rounded-xl bg-white/10 px-3 py-2 text-left backdrop-blur-sm transition hover:bg-white/15"
              >
                <p className="text-[10px] uppercase text-emerald-100/80">Commission</p>
                {commissionLoading ? (
                  <Loader2 className="mt-1 h-5 w-5 animate-spin text-emerald-100" />
                ) : (
